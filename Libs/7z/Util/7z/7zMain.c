...
void SetTimeStamp(HANDLE fhandle, CNtfsFileTime last_time)
{
	...
	if(!GetFileTime(fhandle, &lpCreationTime, &lpLastAccessTime, &lpLastWriteTime))
		return;

	lpLastWriteTime.dwHighDateTime = last_time.High;
	lpLastWriteTime.dwLowDateTime = last_time.Low;
	...
	, const wchar_t *ignore_path_name, const wchar_t *ignore_files[], int ignore_num, Un7z_CallBack call_back)
{
	...
	if (InFile_Open(&archiveStream.file, zfilename))
		return FALSE;

	if(!only_filename || wcslen(only_filename) < 1)
		fullPaths = 1;
	...
	if (res == SZ_OK)
	{
		...
		*/
		...
		if(ignore_path_name)
			pathlen = wcslen(ignore_path_name);

		if(fullPaths && decode_path && wcslen(decode_path) > 0) {
			if(!FileIsDirectory(decode_path))
				MyCreateDir(decode_path);

			if(!FileIsDirectory(decode_path))
				return FALSE;
		}

		SetCurrentDirectoryW(decode_path);
		...
		for (i = 0; i < db.db.NumFiles; i++)
		{
			...
			if(only_filename && wcslen(only_filename) > 0 && wcscmp(only_filename, temp)) {
				continue;
			} else if(ignore_num > 0) {
				int i, bconitnue = 0;
				for(i = 0; i < ignore_num; i++) {
					if(ignore_files[i] && !wcscmp(ignore_files[i], temp) && FileExist(tmpname)) {
						bconitnue = 1;
						break;
					}
				}
			...
			if (!f->IsDir)
			{
				...
				if (res != SZ_OK)
					break;
			} else if(pathlen > 0  && !wcscmp(ignore_path_name, temp))
				continue;
			
			{
				...
				for (j = 0; name[j] != 0; j++)
					...
					SetTimeStamp(outFile.handle, f->MTime);
