...
CAdvDropWnd::CAdvDropWnd( CWnd* pComboParent, list<LIST_ITEM_MOD> &itemlist, DWORD dwACBStyle )
:
...
BEGIN_MESSAGE_MAP(CAdvDropWnd, CWnd)
	//{{AFX_MSG_MAP(CAdvDropWnd)
	ON_WM_CREATE()
	ON_WM_MOUSEMOVE()
	ON_WM_LBUTTONDOWN()
	ON_WM_SIZE()
...
/////////////////////////////////////////////////////////////////////////////
// CAdvDropWnd message handlers

BOOL CAdvDropWnd::PreCreateWindow(CREATESTRUCT& cs) 
{
	//
...
//BOOL CAdvDropWnd::Create(LPCTSTR lpszClassName, LPCTSTR lpszWindowName, DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID, CCreateContext* pContext) 
BOOL CAdvDropWnd::Create(DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID, CCreateContext* pContext) 
{
	// TODO: Add your specialized code here and/or call the base class
	return CWnd::Create(0, 0, dwStyle, rect, pParentWnd, nID, pContext);
}

int CAdvDropWnd::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	m_scrollbar = new CAdvDropScrollBar;
	...
	m_listbox = new CAdvDropListBox(m_pComboParent, m_scrollbar);
	...
	int nDefaultItems = static_cast<CAdvComboBoxMod*>(m_pComboParent)->GetDefaultVisibleItems();
	...
	if( (nVisHeight / nItemHeight) < static_cast<CAdvComboBoxMod*>(m_pComboParent)->GetMinVisibleItems() )
	{
		CRect rcCombo;
...
void CAdvDropWnd::OnMouseMove(UINT nFlags, CPoint point) 
{
	//
	//
	if( m_bResizing )
...
void CAdvDropWnd::OnLButtonDown(UINT nFlags, CPoint point) 
{
	if( m_rcSizeHandle.PtInRect( point ) )
	{
		m_bResizing = true;
...
void CAdvDropWnd::OnLButtonUp(UINT nFlags, CPoint point) 
{
	if( m_bResizing )
	{
		m_bResizing = false;
...
void CAdvDropWnd::OnSize(UINT nType, int cx, int cy) 
{
...
BOOL CAdvDropWnd::DestroyWindow() 
{
	// TODO: Add your specialized code here and/or call the base class
	ReleaseCapture();
...
void CAdvDropWnd::OnShowWindow(BOOL bShow, UINT nStatus) 
{
...
void CAdvDropWnd::OnActivateApp(BOOL bActive, DWORD hTask) 
{
