...
CAdvDropListBox::CAdvDropListBox( CWnd* pComboParent, CAdvDropScrollBar* pScroll )
:
...
CAdvDropListBox::~CAdvDropListBox()
{
	delete m_pListFont;
}


BEGIN_MESSAGE_MAP(CAdvDropListBox, CListBox)
	//{{AFX_MSG_MAP(CAdvDropListBox)
	ON_WM_CREATE()
	ON_WM_MOUSEMOVE()
	ON_WM_LBUTTONUP()
...
/////////////////////////////////////////////////////////////////////////////
// CAdvDropListBox message handlers


int CAdvDropListBox::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	SetFont( static_cast<CAdvComboBoxMod*>(m_pComboParent)->GetFont() ); 
	return 0;
}
...
void CAdvDropListBox::OnMouseMove(UINT nFlags, CPoint point) 
{
	//
	...
	PLIST_ITEM_MOD pItem = (PLIST_ITEM_MOD)GetItemDataPtr(nPos);
	if( (DWORD)pItem != -1 )
	{
...
void CAdvDropListBox::OnLButtonUp(UINT nFlags, CPoint point) 
{
...
void CAdvDropListBox::OnLButtonDown(UINT nFlags, CPoint point) 
{
	//
	// Is mouse within listbox
	CRect rcClient;
	...
	if( nPos != LB_ERR )
	{
		PLIST_ITEM_MOD pItem;
		pItem = (PLIST_ITEM_MOD)GetItemDataPtr( nPos );
		if( pItem->bDisabled )
		{
			return;
		}
...
//	OutputDebugString( str );
	...
	// Destroy dropdown
	ReleaseCapture();
	CAdvComboBoxMod* pList = static_cast<CAdvComboBoxMod*>(m_pComboParent);
...
int CAdvDropListBox::GetBottomIndex()
{
...
void CAdvDropListBox::SetTopIdx(int nPos, BOOL bUpdateScrollbar)
{
	m_nLastTopIdx = nPos;
	SetTopIndex( nPos );
	if( bUpdateScrollbar )
...
void CAdvDropListBox::GetTextSize(LPCTSTR lpszText, int nCount, CSize &size)
{
	CClientDC dc(this);
	int nSave = dc.SaveDC();
	dc.SelectObject( static_cast<CAdvComboBoxMod*>(m_pComboParent)->GetFont() );
...
int CAdvDropListBox::AddListItem( LIST_ITEM_MOD& item )
{
	PLIST_ITEM_MOD pItem = new LIST_ITEM_MOD;
	*pItem = item;
...
void CAdvDropListBox::DrawItem(LPDRAWITEMSTRUCT pDIStruct) 
{
	//
	// Set var's
	...
	pItem = (PLIST_ITEM_MOD)GetItemDataPtr(pDIStruct->itemID);
...
int CAdvDropListBox::CompareItem(LPCOMPAREITEMSTRUCT lpCompareItemStruct) 
{
	// TODO: Replace the next line with your message handler code
	return 0;
}

void CAdvDropListBox::MeasureItem(LPMEASUREITEMSTRUCT lpMeasureItemStruct) 
{
...
BOOL CAdvDropListBox::DestroyWindow() 
{
	PLIST_ITEM_MOD pItem;
	for( int i = 0; i < GetCount(); i++ )
	{
		pItem = (PLIST_ITEM_MOD)GetItemDataPtr( i );
		delete pItem;
	}
...
void CAdvDropListBox::SetDLBStyle(DWORD dwStyle)
{
...
int CAdvDropListBox::SetCurSel(int nSelect)
{
	PLIST_ITEM_MOD pItem = NULL;
	int nCur = GetCurSel();
	...
	if( !m_bSelectDisabled )
	{
		...
		if( nWay < 0 )
		{
			...
			pItem = (PLIST_ITEM_MOD)GetItemDataPtr( nTmp );
			while( (DWORD)pItem != -1 )
			{
				if( !pItem->bDisabled )
				{
	...
	pItem = (PLIST_ITEM_MOD)GetItemDataPtr( nSelect);
	if( (DWORD)pItem != -1 )
	{
		if( pItem->bDisabled )
		{
